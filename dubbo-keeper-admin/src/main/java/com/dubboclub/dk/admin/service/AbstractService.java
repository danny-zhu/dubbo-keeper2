...
public abstract class AbstractService {
    ...
    protected <T extends BasicModel> List<T> filterCategoryDataByServiceKey(ConvertURL2Entity<T> convertURLTOEntity,
                                                                            String category, String serviceKey) {
        return filterCategoryData(convertURLTOEntity, category, Constants.INTERFACE_KEY, Tool.getInterface
                (serviceKey), Constants.GROUP_KEY, Tool.getGroup(serviceKey), Constants.VERSION_KEY, Tool.getVersion
                (serviceKey));
    }

    //通过对某个目录下的数据定义过滤器，过滤出复核条件的数据
    protected <T extends BasicModel> List<T> filterCategoryData(ConvertURL2Entity<T> convertURLTOEntity, String
