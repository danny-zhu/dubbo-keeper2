...
public class OverrideServiceImpl extends AbstractService implements OverrideService {
        ...
        for (Map<Long, URL> urlMap : urlMaps) {
            for (Map.Entry<Long, URL> urlEntry : urlMap.entrySet()) {
                ...
                if (Constants.ANYHOST_VALUE.equals(url.getHost())
                    ...
                    String currentApplication = StringUtils.isEmpty(provider.getApplication()) ? provider.getUsername
                            () : provider.getApplication();
                    if (configApplication == null || Constants.ANY_VALUE.equals(configApplication)
                            || configApplication.equals(currentApplication)) {
                        if ((url.getPort() == 0 || URL.valueOf(provider.getUrl()).getPort() == url.getPort())) {
                            if (url.getPath().equals(Tool.getInterface(provider.getServiceKey())) && StringUtils
                                    .isEquals(url.getParameter(Constants.GROUP_KEY), providerUrl.getParameter
                                            (Constants.GROUP_KEY)) && StringUtils.isEquals(url.getParameter(Constants
                                    .VERSION_KEY), providerUrl.getParameter(Constants.VERSION_KEY))) {
    ...
    public List<Override> listByServiceKey(String serviceKey) {
        ...
        for (Map<Long, URL> urlMap : urlMaps) {
            for (Map.Entry<Long, URL> urlEntry : urlMap.entrySet()) {
                URL url = urlEntry.getValue();
                if (url.getPath().equals(interfaceName) && StringUtils.isEquals(group, url.getParameter(Constants
                        .GROUP_KEY)) && StringUtils.isEquals(version, url.getParameter(Constants.VERSION_KEY))) {
                    overrides.add(SyncUtils.url2Override(new Pair<Long, URL>(urlEntry.getKey(), url)));
                }
            }
        }
    ...
    public Provider configProvider(Provider provider) {
        ...
        for (Override override : overrides) {
            ...
            if (overrideUrl.getParameters().containsKey(Constants.DISABLED_KEY)) {
                url = url.addParameter(Constants.ENABLED_KEY, !Boolean.parseBoolean(overrideUrl.getParameter
