...
public class ApplicationController {
    ...
    public @ResponseBody
    List<AppProvideInfo> getServices() throws UnsupportedEncodingException {
     ...
     * @return
     */
    @RequestMapping("/{id}/consumer-apps.json")
    public @ResponseBody
    List<Application> getConsumerAppByService(@PathVariable("id") long id) {
        Provider provider = providerService.getProviderById(id);
        List<Consumer> consumers = consumerService.listConsumerByConditions(Constants.INTERFACE_KEY, Tool
                .getInterface(provider.getServiceKey()), Constants.VERSION_KEY, Tool.getVersion(provider
                .getServiceKey()), Constants.GROUP_KEY, Tool.getGroup(provider.getServiceKey()));
     ...
     * @return
     */
    @RequestMapping("/{appName}/nodes.json")
    public @ResponseBody
    List<Node> getNodes(@PathVariable("appName") String appName) {
     ...
     * @return
     */
    @RequestMapping("/{appName}/provides.json")
    public @ResponseBody
    List<AppProvideInfo> getProvides(@PathVariable("appName") String appName) throws UnsupportedEncodingException {
        ...
        for (Provider provider : providers) {
            ...
            provideInfo.setGroup(provider.getGroup());
            provideInfo.setId(provider.getId());
            List<Provider> providerList = providerService.listProviderByConditions(Constants.INTERFACE_KEY,
                    provideInfo.getService(), Constants.GROUP_KEY, Tool.getGroup(provider.getServiceKey()), Constants
                            .VERSION_KEY, Tool.getVersion(provider.getServiceKey()));
            for (Provider item : providerList) {
                URL url = URL.valueOf(item.getUrl());
     ...
     * @return
     */
    @RequestMapping("/{appName}/consumes.json")
    public @ResponseBody
    List<AppConsumeInfo> getConsumes(@PathVariable("appName") String appName) throws UnsupportedEncodingException {
        ...
        for (Consumer consumer : consumers) {
            ...
            List<Provider> providers = providerService.listProviderByConditions(Constants.INTERFACE_KEY, Tool
                    .getInterface(consumer.getServiceKey()), Constants.GROUP_KEY, Tool.getGroup(consumer
                    .getServiceKey()), Constants.VERSION_KEY, Tool.getVersion(consumer.getServiceKey()));
     ...
     * @return
     */
    @RequestMapping("/{appName}/consumers.json")
    public @ResponseBody
    List<ConsumerInfo> getConsumers(@PathVariable("appName") String appName) {
     ...
     * @return
     */
    @RequestMapping("/{provider}/{consumer}/consumes.json")
    public @ResponseBody
    List<AppProvideInfo> geConsumeServiceInfoByConsumerAndProvider(@PathVariable("provider") String provider,
                                                                   @PathVariable("consumer") String consumer) throws
        ...
        for (Consumer consumerEntity : consumers) {
            ...
            List<Provider> providers = providerService.listProviderByConditions(Constants.INTERFACE_KEY, Tool
                    .getInterface(consumerEntity.getServiceKey()), Constants.VERSION_KEY, Tool.getVersion
                    (consumerEntity.getServiceKey()), Constants.GROUP_KEY, Tool.getGroup(consumerEntity.getServiceKey
                    ()), Constants.APPLICATION_KEY, provider);
            if (providers.size() > 0) {
                AppProvideInfo provideInfo = new AppProvideInfo();
                Provider providerEntity = providers.get(0);
                provideInfo.setServiceKey(URLEncoder.encode(URLEncoder.encode(providerEntity.getServiceKey(),
                        "UTF-8"), "UTF-8"));
