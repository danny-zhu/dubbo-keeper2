...
serviceProvider.controller("viewProvider",function($scope,$httpWrapper,$routeParams,$breadcrumb,$httpWrapper,$menu){
    ...
    $httpWrapper.post({url:"provider/"+$routeParams.id+"/provider-detail.json",success:function(data){
        $scope.provider=data;
...
serviceProvider.controller("editProvider",function($scope,$http,$routeParams,$breadcrumb,$dialog,$httpWrapper,$menu){
    ...
    $httpWrapper.post({
        url:"provider/"+$routeParams.id+"/provider-detail.json",
    ...
    $scope.update=function(){
        $dialog.confirm({content:"确认提交修改内容？",callback:function(){
            $httpWrapper.post({
                url:"provider/edit-provider.json",
...
serviceProvider.controller("serviceProviders",function($scope,$http,$routeParams,$queryFilter,$breadcrumb,$dialog,$httpWrapper,$menu){
    ...
    var requestUrl = "provider/"+encodeURIComponent($routeParams.serviceKey)+"/providers.json";
    if($routeParams.host){
        requestUrl="provider/"+$routeParams.application+"/"+$routeParams.host+"/providers.json";
    }
    ...
    $scope.operation=function(type,provider){
        var submit = function(){
            $httpWrapper.post({
                url:"provider/"+provider.id+"/"+type+"/operate.json",
                success:function(data){
    ...
    $scope.batchOperation=function(type){
        ...
        var submit=function(){
            $httpWrapper.post({
                url:"provider/"+type+"/batch-operate.json",
	...
	(function(){
        $httpWrapper.get({
            ...
            success:function(mdText){
            	$httpWrapper.post({
            		url:"provider/"+encodeURIComponent($routeParams.serviceKey)+"/service-readme.json",
                    success:function(data){
                    	...
                    	if(data.providers.length > 0){
                    		...
                    		var libs = {
                    				default: [
										...
										'org.springframework/dubbo-core/2.5.6.SEC03',
