...
override.controller('editOverride',function($scope,$httpWrapper,$routeParams,$queryFilter,$breadcrumb,$menu,$dialog){
    ...
    $httpWrapper.post({
        url:"override/provider/"+encodeURIComponent(encodeURIComponent($scope.serviceKey))+"/methods.json",
        success:function(data){
            $scope.methods=data;
            ...
            if($routeParams.id){
                ...
                $httpWrapper.post({
                    url:"override/"+$scope.id+"/detail.json",
                    success:function(data){
    ...
    $scope.save=function(){
        ...
        $httpWrapper.post({
            url:"override/provider/"+encodeURIComponent(encodeURIComponent($scope.serviceKey))+"/saveOverride.json",
            data:item,
            success:function(data){
...
override.controller('providerOverrides',function($scope,$httpWrapper,$routeParams,$queryFilter,$breadcrumb,$menu,$dialog){
    ...
    var queryConfig=function(){
        $httpWrapper.post({
            url:"override/provider/"+encodeURI($routeParams.serviceKey)+"/list.json",
            success:function(data){
                $scope.details=data;
                if(!data||data.length<=0){
                    $scope.isEmpty=true;
    ...
    var queryWeightConfig = function(){
        $httpWrapper.post({
            url:"override/provider/"+encodeURIComponent($routeParams.serviceKey)+"/weight-list.json",
            success:function(data){
                $scope.details=data;
                if(!data||data.length<=0){
                    $scope.isEmpty=true;
    ...
    var queryLoadBalance=function(){
        $httpWrapper.post({
            url:"override/provider/"+encodeURIComponent($routeParams.serviceKey)+"/loadbalance-list.json",
            success:function(data){
                $scope.details=data;
                if(!data||data.length<=0){
                    $scope.isEmpty=true;
    ...
    $scope.operation=function(type,item){
        $dialog.confirm({
            ...
            callback:function(){
                $httpWrapper.post({
                    url:"/override/"+item.id+"/"+type+".json",
                    success:function(data){
                        if(data.result==ajaxResultStatu.SUCCESS){
                            $dialog.info({
                                content:"成功更新该配置！",
