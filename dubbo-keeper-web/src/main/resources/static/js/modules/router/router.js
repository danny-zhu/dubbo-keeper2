...
router.controller('scriptEdit', function ($scope,$httpWrapper,$routeParams,$queryFilter,$breadcrumb,$menu,$dialog) {
    ...
    $scope.save= function () {
        ...
        if(script&&script!=""){
            ...
            $httpWrapper.post({
                url:$scope.id?"route/update.json":"route/create.json",
                data:$scope.item,
                success: function (data) {
...
router.controller('conditionEdit',function($scope,$httpWrapper,$routeParams,$queryFilter,$breadcrumb,$menu,$dialog){
    ...
    $scope.thenList=[];
    $httpWrapper.post({
        url:"override/provider/"+encodeURIComponent($routeParams.serviceKey)+"/methods.json",
        success:function(data){
            $scope.methods=data;
        }
    });
...
router.controller('providerRoutes', function ($scope,$httpWrapper,$routeParams,$queryFilter,$breadcrumb,$menu,$dialog) {
    ...
    var refreshData=function(){
        $httpWrapper.post({
            url:"route/provider/"+encodeURIComponent($routeParams.serviceKey)+"/list.json",
            success:function(data){
                $scope.details=data;
                if(!data||data.length<=0){
                    $scope.isEmpty=true;
